...
(defn echo-handler
  [req]
  (if-let [socket (try
                    ;; yields a websocket connection if the
                    ;; request can be so converted
                    @(http/websocket-connection req)
                    (catch Exception e
                      nil))]
    ;; connect the stream to itself to echo back messages
    (s/connect socket socket)))

;; the above will block while the socket handshake completes
;; the following will do the handshake asynchronously
...
(defn echo-handler
  ...
  (-> (http/websocket-connection req)
      (d/chain
       (fn [socket]
         (s/connect socket socket))) ; attempt a connection
      (d/catch
          (fn [_]
            non-websocket-request)))) ; yield non-ws-req if it fails

;; another asynchronous implementation that resembles
;; the synchronous one slightly more closely

(defn echo-handler [req]
  ;; let-flow allows deferred values bound in the clause to be
  ;; treated as though they were realized. Computations using these values
  ;; are only executed once all let-bound values are computed.
